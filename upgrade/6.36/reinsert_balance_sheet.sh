...
(
cat << all_done
delete from $process where process = 'balance_sheet';
insert into $process (process,command_line,notepad,html_help_file_anchor,post_change_javascript,process_set_display,appaserver_yn,process_group,preprompt_help_text) values ('balance_sheet','balance_sheet \$session \$login_name \$role \$process fund as_of_date prior_year_count fund_aggregation subclassification_option output_medium',null,null,null,null,null,'output',null);
delete from $process_parameter where process = 'balance_sheet';
insert into $process_parameter (process,folder,attribute,prompt,drop_down_prompt,display_order,drop_down_multi_select_yn,preprompt_yn,populate_drop_down_process,populate_helper_process) values ('balance_sheet','null','null','as_of_date','null','1',null,null,null,null);
insert into $process_parameter (process,folder,attribute,prompt,drop_down_prompt,display_order,drop_down_multi_select_yn,preprompt_yn,populate_drop_down_process,populate_helper_process) values ('balance_sheet','null','null','null','finance_output_medium','9',null,null,null,null);
insert into $process_parameter (process,folder,attribute,prompt,drop_down_prompt,display_order,drop_down_multi_select_yn,preprompt_yn,populate_drop_down_process,populate_helper_process) values ('balance_sheet','null','null','null','subclassification_option','3',null,null,null,null);
insert into $process_parameter (process,folder,attribute,prompt,drop_down_prompt,display_order,drop_down_multi_select_yn,preprompt_yn,populate_drop_down_process,populate_helper_process) values ('balance_sheet','null','null','prior_year_count','null','2',null,null,null,null);
delete from $javascript_processes where process = 'balance_sheet';
delete from $process_generic_output where process = 'balance_sheet';
