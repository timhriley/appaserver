...
insert into $attribute (attribute,attribute_datatype,width,float_decimal_places,post_change_javascript,hint_message,on_focus_javascript_function,lookup_histogram_output_yn,lookup_time_chart_output_yn,appaserver_yn) values ('feeder_phrase','text','255',null,null,null,null,null,null,null);
...
delete from $folder_attribute where folder = 'feeder_load_row';
insert into $folder_attribute (folder,attribute,primary_key_index,display_order,omit_insert_yn,omit_insert_prompt_yn,additional_unique_index_yn,additional_index_yn,omit_update_yn,lookup_required_yn,insert_required_yn) values ('feeder_load_row','feeder_account','4',null,null,null,'y',null,null,null,null);
...
insert into $role_folder (folder,role,permission) values ('feeder_load_row','supervisor','update');
delete from $javascript_folders where folder = 'feeder_load_row';
...
insert into $role_operation (folder,role,operation) values ('feeder_load_row','bookkeeper','detail');
insert into $role_operation (folder,role,operation) values ('feeder_load_row','supervisor','delete');
insert into $role_operation (folder,role,operation) values ('feeder_load_row','supervisor','detail');
all_done
) | sql.e 2>&1 | grep -iv duplicate

table_name=feeder_load_row
echo "drop table if exists $table_name;" | sql.e
echo "create table $table_name (full_name char (60) not null,street_address char (60) not null,transaction_date_time datetime not null,feeder_account char (40) not null,feeder_date date,feeder_description char (140),feeder_amount double (12,2),feeder_row_balance double (12,2),row_number integer,feeder_phrase char (255),feeder_load_date_time datetime) engine MyISAM;" | sql.e
