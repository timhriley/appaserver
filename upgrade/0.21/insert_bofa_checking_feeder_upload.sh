...
insert into process (process,command_line,notepad,html_help_file_anchor,execution_count,post_change_javascript,process_group,process_set_display,javascript_filename,preprompt_help_text) values ('BofA_checking_upload','feeder_upload_csv \$process \$login_name bank_of_america_checking filename 1 2 3 0 4 0 reverse_order_yn account_end_balance execute_yn','<ol><li>Download your checking transactions from Bank of America.<li>Choose the file type of Excel.<li>Hint: do not first load them into Excel and then save them to your computer. Instead, save them and then, optionally, load them into Excel.<li> Use a begin date as the end date of your last download. The end date of your last download will display if you submit this form without a file. Use \'Final Feeder Date\' as the begin date.<li>Hint: [Right Click] the browser\'s tab above and select \'Duplicate Tab\'. Use the new PredictBooks instance to:<ul><li>Feeder --> Insert Expense Transaction or<li>Feeder --> Insert Feeder Phrase</ul></ol>',null,'48',null,'feeder',null,null,null);
