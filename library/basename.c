...
char *basename_directory( char *filename )
{
	static char base_name[ 128 ];
	char local_buffer[ 512 ];
	char *base_name_ptr;

	string_strcpy( local_buffer, filename, 512 );
...
char *basename_filename( char *filename )
{
	return
...
char *basename_extension( char *filename )
{
	...
	*extension = '\0';
	...
	count = string_character_count( '.', filename );

	if ( count )
	{
		piece( extension, '.', filename, count );
	}
