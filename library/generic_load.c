				...
				int primary_key_time_offset )
{
	...
	date_increment_days( date, 1.0, date_utc_offset() );
				...
				int skip_header_rows_integer )
{
	...
	if ( ! ( primary_key_list_string =
		generic_load_get_primary_key_list_string(
			generic_load_folder->
				generic_load_attribute_list ) ) )
	{
		fprintf( stderr,
"ERROR in %s/%s()/%d: cannot generic_load_get_primary_key_list_string()\n",
			 __FILE__,
			 __FUNCTION__,
			 __LINE__ );
		return 0;
...
#ifdef DEBUG_MODE
delete_output_process = "html_paragraph_wrapper.e";
#else
delete_output_process = "sql.e";
#endif

	sprintf( sys_string,
"delete_statement.e table=%s field=%s delimiter='^' | %s",
		 table_name,
		 primary_key_list_string,
		 delete_output_process );
	...
	while( get_line( input_buffer, input_file ) )
	{
		...
		if ( generic_load_skip_header_rows( skip_header_rows_integer ) )
			continue;
		...
		list_rewind( generic_load_folder->generic_load_attribute_list );
		...
		do {
			...
			if ( !list_at_head( generic_load_folder->
						generic_load_attribute_list ) )
			{
				fprintf( output_pipe, "^" );
			}
		...
		} while( list_next( generic_load_folder->
					generic_load_attribute_list ) );
		fprintf( output_pipe, "\n" );
	}
...
} /* generic_load_delete_from_database() */
#endif
