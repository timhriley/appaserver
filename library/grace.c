				...
				boolean symbols )
{
	...
	do {
		dataset = list_get_pointer( datatype->dataset_list );
					...
					LIST *anchor_graph_list )
{
	...
	do {
		graph = list_get_pointer( graph_list );
		anchor_graph = (GRACE_GRAPH *)list_get_pointer(
						anchor_graph_list );

		graph_datatype = (GRACE_DATATYPE *)
			list_get_first_pointer( graph->datatype_list );

		anchor_graph_datatype = (GRACE_DATATYPE *)
			list_get_first_pointer( anchor_graph->datatype_list );
...
double grace_get_minimum_y_double( LIST *datatype_list )
{
	...
	do {
		datatype = (GRACE_DATATYPE *)list_get_pointer( datatype_list );
...
double get_average_difference_double( LIST *double_list )
{
	...
	do {
		...
		{
			next_double_pointer = (double *)list_get_pointer(
							double_list );
			sum_difference += 
...
*/
	...
	do {
		grace_datatype_overlay_input =
			(GRACE_DATATYPE_OVERLAY_INPUT *)list_get_pointer(
...
"ERROR in %s.%s(): graph list has different count than anchor graph list\n",
	...
	do {
		graph = (GRACE_GRAPH *)list_get_pointer( graph_list );

		anchor_graph = (GRACE_GRAPH *)
				list_get_pointer( anchor_graph_list );
				...
				boolean dataset_no_cycle_color )
{
	...
	graph = (GRACE_GRAPH *)list_get_first_pointer( graph_list );
...
void grace_set_first_graph_top_of_second( LIST *graph_list )
{
	...
	list_rewind( graph_list );
	first_grace_graph = list_get_pointer( graph_list );
	list_next( graph_list );
	second_grace_graph = list_get_pointer( graph_list );
...
void grace_graph_increment_line_color( LIST *datatype_list )
{
	...
	do {
		grace_datatype = list_get_pointer( datatype_list );
