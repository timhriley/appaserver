...
void output_dictionary_string_as_hidden(
			char *dictionary );
...
LIST *appaserver_library_datatype_name_list(
			char *application );
...
char **get_system_folder_list(
			void );

char **appaserver_library_system_folder_list(
			void );
...
char *build_multi_attribute_key(
			LIST *key_list,
			char delimiter );
...
char *appaserver_library_whoami(
			void );

boolean appaserver_library_from_preprompt(
			DICTIONARY *dictionary );
...
char *appaserver_library_sort_attribute_name(
			LIST *attribute_list );
