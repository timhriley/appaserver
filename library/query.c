				...
				RELATED_FOLDER *root_related_folder )
{
	...
	if ( folder_name )
	{
		...
		else
		{
			...
			if ( list_length( mto1_join_folder_name_list ) )
			{
			  query->mto1_join_folder_list =
			  query_with_folder_name_list_get_mto1_join_folder_list(
				application_name,
...
{
char msg[ 65536 ];
sprintf( msg, "\n%s/%s()/%d: for folder = %s, query_attribute_list = %s\n",
__FILE__,
__FUNCTION__,
__LINE__,
folder->folder_name,
query_attribute_list_display(	folder->folder_name,
				query_output->query_attribute_list ) );
m2( folder->application_name, msg );
}
*/
				...
				LIST *append_isa_attribute_list )
{
	...
	if ( !list_length( append_isa_attribute_list ) ) return (char *)0; 
				...
				DICTIONARY *preprompt_dictionary )
{
	...
	query_drop_down = query_drop_down_new( folder_name );
...
QUERY_DROP_DOWN *query_get_primary_data_drop_down(
				LIST *exclude_attribute_name_list,
...
} /* query_get_drop_down_dictionary_data() */
...
/*
		ptr += sprintf( ptr,
				 "\nWarning: empty query_drop_down_list." );
*/
		return strdup( buffer );
	}
			...
			boolean combine_date_time )
{
	...
	{
		*drop_down_where_clause =
			query_get_folder_drop_down_where_clause(
				query_drop_down_list,
				application_name,
				folder_name );
...
} /* query_folder_get_where_clause() */

char *query_get_where_clause(
			char **drop_down_where_clause,
			char **attribute_where_clause,
			LIST *query_drop_down_list,
			LIST *query_attribute_list,
...
} /* query_get_date_time_between_attributes() */
...
} /* query_with_sort_multi_attribute_get_order_clause() */
...
} /* query_parse_multi_attribute_get_order_clause() */
					...
					ROLE *role )
{
	...
	if ( !list_rewind( mto1_folder_name_list ) ) return (LIST *)0;

	mto1_join_folder_list = list_new();

	do {
		folder_name = list_get( mto1_folder_name_list );
