#!/bin/bash
# ----------------------------------------------------------------
# src_predictive/program_journal_aggregate_account.sh
# ----------------------------------------------------------------
#
...
select="${common_select},sum(credit_amount)"

order="sum(credit_amount) desc"

echo "select $select from $from where $where group by $common_select order by $order;" | sql
}
...
select="${common_select},sum(debit_amount)"

order="sum(debit_amount) desc"
