...
#include "process_generic_exceedance.h"
...
int main( int argc, char **argv )
{
	...
	PROCESS_GENERIC_EXCEEDANCE *process_generic_exceedance;
	...
	{
		fprintf( stderr,
"Usage: %s login_name process process_set output_medium dictionary\n",
			 argv[ 0 ] );
		exit ( 1 );
	}
	...
	process_generic_exceedance =
		process_generic_exceedance_new(
			application_name,
			login_name,
			process_name,
			process_set_name,
		...
		output_medium == table )
	{
		html_table_output(
			process_generic_exceedance->
				process_generic_exceedance_table->
				html_table,
	...
	{
		LIST *grace_graph_list = list_new();

		list_set(
			grace_graph_list,
			process_generic_exceedance->
				process_generic_exceedance_grace->
				grace_generic->
				grace_graph );
		...
		output_medium == spreadsheet )
	{
		...
		output_file =
			...
			appaserver_output_file(
				process_generic_exceedance->
					process_generic_exceedance_spreadsheet->
					spreadsheet->
					output_filename );
		...
		output_medium == googlechart )
	{
		google_generic_create(
			process_generic_exceedance->
				process_generic_exceedance_google->
				google_generic->
				google_filename->
